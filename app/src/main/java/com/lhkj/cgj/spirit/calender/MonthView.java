...
import com.lhkj.cgj.utils.DateUtils;
...
public abstract class MonthView extends View {
    ...
    protected int tonchYear, tonchMonth, touchDay;
    private ADCircleCalendarView.DayData firstDay, afterDay;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 但是郭士超给改好了，不一定好用，可以先用着
        NUM_ROWS = 6;
        int weekNumber = DateUtils.getFirstDayWeek(selYear, selMonth);
        // 这里郭士超修改过，是显示高度之类有关的东西
        if (1 == weekNumber) {
            NUM_ROWS = 6;
        } else {
            NUM_ROWS = getMonthRowNumber(selYear, selMonth);
        }
    ...
    private void drawDate(Canvas canvas, int year, int month, int startX, int startY) {
        ...
        for (int day = 0; day < mMonthDays; day++) {
            column = (day + weekNumber - 1 - startDay) % 7;
            row = (day + weekNumber - 1 - startDay) / 7;
            if (1 == weekNumber) {
                if (column < 0) {
                    column = 6;
                } else {
             ...
             * 绘制当前日期背景
             * */
//            if (column-(startDay)<0){
//                row++;
//                drawBG(canvas, column-(6-startDay), row,year, month, daysString[row][column]);
////            drawDayArr(canvas, column, row,year, month, daysString[row][column],firstDay,afterDay);
//                drawDecor(canvas, column-(6-startDay), row, year, month, daysString[row][column]);
//                drawRest(canvas, column-(6-startDay), row, year, month, daysString[row][column]);
//                drawText(canvas, column-(6-startDay), row, year, month, daysString[row][column]);
//            }else {
...
//            }
    ...
    //多选
    protected abstract void drawDayArr(Canvas canvas, int column, int row, int year, int month, int day, ADCircleCalendarView.DayData firstDay, ADCircleCalendarView.DayData afterDay);
     ...
     * @return
     */
    protected Bitmap getcalendarBit(int year, int month, int day) {
        if (calendarInfos == null || calendarInfos.size() == 0) return null;
        for (CalendarInfo calendarInfo : calendarInfos) {
            if (calendarInfo.day == day && calendarInfo.month == month + 1 && calendarInfo.year == year) {
                return calendarInfo.bitmap;
            }
        }
        return null;
    }
    ...
    public void setDayArr(ADCircleCalendarView.DayData firstDay, ADCircleCalendarView.DayData afterDay) {
